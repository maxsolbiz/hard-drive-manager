...
import hardDriveIcon from "../assets/harddrive.png";
...
const DashboardLayout = () => {
  ...
  // Fetch drive detection data from your detection endpoint (adjust URL if needed)
  useEffect(() => {
  ...
  // Normalize device names to merge data
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex flex-col">
      ...
      {/* Main Content: Single-column grid of drive cards */}
      <main className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 gap-6">
          {combinedDriveData.map((drive, index) => (
            <DashboardCard key={index} drive={drive} icon={hardDriveIcon} />
          ))}
          {combinedDriveData.length === 0 && (
            <p className="text-center text-gray-500">No drive data available.</p>
          )}
        </div>
      </main>
